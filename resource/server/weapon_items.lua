...
---@param id number
---@return number
local function getMk2TintIndex(id)
    return mk2Tints[id] or id
end
...
exports('setweapontint', function(event, item, inventory)
    ...
    if not weapon then
        bridge.notifications.notify(src, { description = locale('WeaponTints.NotEquipped'), type = 'error' })
        return false
    end
    ...
    if isMk2Weapon(weapon.name) then
        assignedTint = getMk2TintIndex(item.server.tint)
    end
